...
const Rents = () => {
  ...
  const handleReturnButton = async (rentId) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/rents/returning/${rentId}`,
        {
  ...
  const handleApproveButton = async (rentId) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/rents/approve/${rentId}`,
        {
          rentStatus: "Approved",
  ...
  const handleCancelButton = async (rentId) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/rents/cancelled/${rentId}`,
        {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const token = localStorage.getItem("token");
        const usersResponse = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/users`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setUsers(usersResponse.data);
        const rentsResponse = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        setRents(rentsResponse.data);
      } catch (error) {
          ...
          <Button
            onClick={async () => {
              ...
              try {
                setLoading(true);
                const response = await axios.put(
                  `${process.env.REACT_APP_API_BASE_URL}/rents/renting/${selectedRent.rentId}`,
                  {
                    rentCondition: "Rented",
                    expectedReturnDate,
                    rentBy: `${user.fname} ${user.lname}`,
