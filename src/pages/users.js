...
const Users = () => {
  ...
  const fetchStaff = async () => {
    try {
      ...
      if (filter.positionId) {
        url = `${process.env.REACT_APP_API_BASE_URL}/users/${filter.positionId}`;
      }
  ...
  const fetchPositions = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${process.env.REACT_APP_API_BASE_URL}/positions`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleActivateUser = async (userId) => {
    try {
      ...
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/users/activateUser/${userId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeactivateUser = async (userId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.put(
        `${process.env.REACT_APP_API_BASE_URL}/users/deactivateUser/${userId}`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
