  ...
  const fetchCountsByUser = async () => {
    try {
      const [
        ...
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents/count_rented_request_by_user?userId=${userId}`, { headers: { Authorization: `Bearer ${user.token}` } }),
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents/count_pending_by_user?userId=${userId}`, { headers: { Authorization: `Bearer ${user.token}` } }),
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents/count_rented_by_user?userId=${userId}`, { headers: { Authorization: `Bearer ${user.token}` } }),
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents/count_returned_by_user?userId=${userId}`, { headers: { Authorization: `Bearer ${user.token}` } }),
        axios.get(`${process.env.REACT_APP_API_BASE_URL}/rents/count_rejected_by_user?userId=${userId}`, { headers: { Authorization: `Bearer ${user.token}` } })
      ]);
          ...
          <div className="card border-left-secondary shadow mb-4">
            <div className="card-body">
              <h5 className="text-center font-weight-bold text-secondary mb-3">
                My Historical Lab Asset's Movement
              </h5>
              <hr />
              ...
              <div className="row">
                ...
                <div className="col-md-6 mb-3">
                  <div className="text-center">
                    <Link to="/my_own_history" className="h6 font-weight-bold text-gray-800">
